...
public class PngTitler {
	...
	static public BufferedImage process(BufferedImage im, List<String> titles) throws IOException {
		if (titles != null && titles.size() > 0) {
			im = addTitle(im, titles);
		}
		return im;

	}

	static private BufferedImage addTitle(BufferedImage im, List<String> titles) throws IOException {

		final Graphics2D oldg2d = im.createGraphics();
		final TextBlock title = TextBlockUtils.create(titles, normalFont, Color.BLACK, HorizontalAlignement.CENTER);
