...
public final class CucaDiagramPngMaker {
	...
	public List<File> createPng(File pngFile, String... dotStrings) throws IOException, InterruptedException {
		...
		if (diagram.isRotation()) {
			new PngRotation(pngFile, diagram.getSource());
		}
		new PngSizer(pngFile, diagram.getMinwidth(), diagram.getSource());
		return new PngSplitter(pngFile, diagram.getHorizontalPages(), diagram.getVerticalPages(), diagram.getSource()).getFiles();
	}
	...
	public void createPng(OutputStream os, String... dotStrings) throws IOException, InterruptedException {
		...
		try {
			...
			final String dotString = dotMaker.createDotString(staticFiles.getStaticImages(), imageFiles);
	...
	protected GraphvizMaker createDotMaker(String... dotStrings) {
