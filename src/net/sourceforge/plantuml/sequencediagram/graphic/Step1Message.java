...
class Step1Message {
	...
	Step1Message(Graphics2D g2d, Message message, StepData stepData, ConstraintSet constraintSet, double freeY) {
		this.g2d = g2d;
		...
		this.lp1 = stepData.getParticipants().get(message.getParticipant1());
		this.lp2 = stepData.getParticipants().get(message.getParticipant2());
		...
		this.type = p1.equals(p2) ? getSelfArrowType(message) : getArrowType(message, x1, x2);
		...
		if (p1.equals(p2)) {
			...
			this.messageArrow = new MessageArrow(freeY, stepData.getSkin(), stepData.getSkin().createComponent(type,
					getLabelOfMessage(message)), lp1, lp2);
		...
		if (message.getNote() == null) {
			...
			note = stepData.getSkin().createComponent(ComponentType.NOTE, message.getNote());
		}
	...
	public double prepareMessage() {
		...
		stepData.getEvents().put(message, graphic);
	...
	private Arrow createArrow() {
		if (message.getNote() != null && p1.equals(p2)) {
			return new ArrowAndNoteBox(g2d, new MessageSelfArrow(freeY, stepData.getSkin(), stepData.getSkin()
					.createComponent(type, getLabelOfMessage(message)), lp1), note, message.getNotePosition());
