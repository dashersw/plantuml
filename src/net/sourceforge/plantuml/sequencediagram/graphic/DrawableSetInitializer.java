...
class DrawableSetInitializer {
	...
	public DrawableSetInitializer(Skin skin, boolean showTail) {
		this.drawableSet = new DrawableSet(skin);
	...
	public DrawableSet createDrawableSet(Graphics2D g2d, List<String> title) {
		...
		if (title != null && title.size() > 0) {
			compTitle = drawableSet.getSkin().createComponent(ComponentType.TITLE, title);
		...
		drawableSet.setDimension(new Dimension2DDouble(width, getTotalHeight(g2d)), (width - diagramWidth) / 2);
	...
	private void prepareGrouping(Graphics2D g2d, Grouping m) {
		...
		if (m.getType() == GroupingType.START) {
			...
			final Component body = drawableSet.getSkin().createComponent(ComponentType.GROUPING_BODY, null);
			final Component tail = drawableSet.getSkin().createComponent(ComponentType.GROUPING_TAIL, null);
	...
	private void prepareLiveEvent(Graphics2D g2d, LifeEvent lifeEvent) {
		...
		if (lifeEvent.getType() == LifeEventType.CREATE) {
			...
			final Component head = drawableSet.getSkin()
					.createComponent(ComponentType.PARTICIPANT_HEAD, p.getDisplay());
	...
	private void prepareParticipant(Graphics2D g2d, Participant p) {
		...
		if (p.getType() == ParticipantType.PARTICIPANT) {
			final Component head = drawableSet.getSkin()
					.createComponent(ComponentType.PARTICIPANT_HEAD, p.getDisplay());
			final Component line = drawableSet.getSkin()
					.createComponent(ComponentType.PARTICIPANT_LINE, p.getDisplay());
			final Component tail = drawableSet.getSkin()
					.createComponent(ComponentType.PARTICIPANT_TAIL, p.getDisplay());
			box = new ParticipantBox(head, line, tail, this.freeX);
		} else if (p.getType() == ParticipantType.ACTOR) {
			final Component head = drawableSet.getSkin().createComponent(ComponentType.ACTOR_HEAD, p.getDisplay());
			final Component line = drawableSet.getSkin().createComponent(ComponentType.ACTOR_LINE, p.getDisplay());
			final Component tail = drawableSet.getSkin().createComponent(ComponentType.ACTOR_TAIL, p.getDisplay());
			box = new ParticipantBox(head, line, tail, this.freeX);
		} else {
			throw new IllegalArgumentException();
		}

		final Component comp = drawableSet.getSkin().createComponent(ComponentType.ALIVE_LINE, null);
