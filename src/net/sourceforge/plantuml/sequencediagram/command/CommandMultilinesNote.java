...
public class CommandMultilinesNote extends CommandMultilines<SequenceDiagram> implements CommandNote {

	public CommandMultilinesNote(final SequenceDiagram sequenceDiagram) {
		super(sequenceDiagram, "(?i)^(note|hnote|rnote)\\s+(right|left|over)\\s+(?:of\\s+)?([\\p{L}0-9_.@]+|\"[^\"]+\")\\s*(#\\w+)?$");
	}
	...
	public CommandExecutionResult execute(List<String> lines) {
		...
		final Participant p = getSystem().getOrCreateParticipant(StringUtils.eventuallyRemoveStartingAndEndingDoubleQuote(line0.get(2)));

		final NotePosition position = NotePosition.valueOf(line0.get(1).toUpperCase());
