...
public class Base72Encoder implements URLEncoder {

	private static final int GROUP_BIT = 37;
	private static final int GROUP_CHAR = 6;
		...
		for (int k = 0; k < result.length; k++) {
			for (int i = 0; i < 8; i++) {
				...
				if (bn1 * GROUP_CHAR < s.length()) {
					...
					final long x = Base72.decode(sub);
					test = (x & (1L << bn2)) != 0;
				}
